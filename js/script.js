...
checkoutBtn.addEventListener('click', () => {
    ...
    const whatsappURL = `https://api.whatsapp.com/send?phone=5491136453000&text=${encodeURIComponent(whatsappMessage)}`;
    window.open(whatsappURL, '_blank');
});

// Inicialización de la aplicación
const init = async () => {
    // Carga los productos desde JSONBin.io
    const loadedProducts = await loadProducts();

    // Si no hay productos en el bin (primera vez), carga los productos iniciales
    if (loadedProducts.length === 0) {
        products = initialProducts;
        // Guarda los productos iniciales en el bin
        await saveProducts(products);
    } else {
